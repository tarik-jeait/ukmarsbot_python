...
class Cell:
    ...
    def neighbour(self,heading,direction):
        #Logger.log("cell [%d][%d] neighbour:%s-%s"%(self.x,self.y,heading,direction.name))
    ...
    def is_exit(self,heading):
        if self.walls[heading] == WallState.EXIT or self.walls[heading] == WallState.UNKNOWN:
            return 1
        else:
            return 0

    def set_wall_state(self,heading,state):
        Logger.log("set_wall_state:[%d][%d]:%s-%s-current state:%s"%(self.x,self.y,heading,state,self.walls[heading]))
