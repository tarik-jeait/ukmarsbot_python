...
def photoread(board):
    ...
    time.sleep(0.02)
    leftside = board.LEFT_SENSOR.read_u16()
    leftfront = board.LEFT_FRONT_SENSOR.read_u16()
    rightfront = board.RIGHT_FRONT_SENSOR.read_u16()
    rightside = board.RIGHT_SENSOR.read_u16()
    board.TRIGGER_PIN.value(0)
    return

# Line follower code
def widelinefollow(board):
    ...
    while True:
        ...
        if (leftfront < 10000):            # put LED indicators on if white line seen
            board.LEFT_LED.value(True)
        else :
            board.LEFT_LED.value(False)
            
        if (rightfront < 10000):
            board.RIGHT_LED.value(True)
        else :
            board.RIGHT_LED.value(False)       
...
def lineFollow(board): #Black Line Follow on White surface
